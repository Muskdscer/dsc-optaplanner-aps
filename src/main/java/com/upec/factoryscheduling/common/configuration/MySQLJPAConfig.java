...
public class MySQLJPAConfig {
    ...
    public LocalContainerEntityManagerFactoryBean mySqlSessionFactory(@Qualifier(value = "mySqlJpaVendorAdapter") JpaVendorAdapter jpaVendorAdapter) {
        ...
        // 注意：这里是 JPA Entity 扫描范围（不是 Repository 扫描范围）
    ...
    /**
     * 兼容历史代码：大量服务仍然使用 @Transactional("oracleTransactionManager")。
     * 当前项目以 MySQL 为主数据源时，提供一个同指向的事务管理器别名，避免启动失败。
     */
//    @Bean(name = "oracleTransactionManager")
//    public PlatformTransactionManager oracleTransactionManagerAlias(
//            @Qualifier("mySqlSessionFactory") LocalContainerEntityManagerFactoryBean entityManagerFactory) {
//        JpaTransactionManager transactionManager = new JpaTransactionManager();
//        transactionManager.setEntityManagerFactory(entityManagerFactory.getObject());
//        transactionManager.setDefaultTimeout(300);
//        return transactionManager;
//    }
    ...
    /**
     * 兼容历史代码：大量类使用 @Qualifier("oracleTemplate")
     * 当前以 MySQL 为主数据源时，提供一个同指向的 JdbcTemplate 别名，避免注入失败。
     */
    @Bean("oracleTemplate")
    public JdbcTemplate oracleTemplateAlias() {
